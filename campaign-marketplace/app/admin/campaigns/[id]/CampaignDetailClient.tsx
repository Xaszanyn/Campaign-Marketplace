...
export function CampaignDetailClient({ campaignId }: { campaignId: string }) {
  ...
  const { data: overview, refetch: refetchOverview } = trpc.campaign.overview.useQuery({
    id: campaignId,
  });

  const { data: pending = [], refetch: refetchPending } = trpc.submission.listByCampaign.useQuery({
    campaign: campaignId,
    status: "pending",
  });
